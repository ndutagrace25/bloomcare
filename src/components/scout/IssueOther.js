...
const IssueOther = ({
  ...
  const others = realmIssueTypes.filter(issueT => {
    return issueT.issue_type_name === 'Others';
  });

  const othersName = others[0].issue_type_name;
  const othersId = others[0].id;

  const otherIssues = realmIssues.filter(issue => {
    return issue.issue_type_id === othersId;
  });

  const allIssues = otherIssues.map(issue => (
    <View style={styles.rowContainer} key={issue.id}>
      <View style={styles.rowTop}>
        <CheckBox
          ...
          onPress={() => {
            setIssue(othersId, issue.id);
            setIssueValueWithoutCategory(othersId, '5', issue.id);
          }}
        />
      </View>
          ...
          {radio_props.map((cat, i) => (
            <RadioButton labelHorizontal={true} key={i}>
              <RadioButtonInput
                ...
                onPress={value =>
                  setIssueValueWithoutCategory(othersId, value, issue.id)
                }
                borderWidth={1}
  ...
  return (
    <ScrollView
      ...
      <View style={styles.title}>
        <Text style={styles.titleText}>{othersName}</Text>
      </View>
      {allIssues}
    </ScrollView>
  );
